...
import (
	...
	store "github.com/alexgridx/kinesis-consumer/store/mysql"
)
...
func main() {
	...
	// mysql checkpoint
	mysqlStore, err := store.New(*app, *table, *connStr)
	if err != nil {
	...
	if err := mysqlStore.Shutdown(); err != nil {
		log.Fatalf("mysqlStore shutdown error: %v", err)
	}
}
