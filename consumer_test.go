...
func TestScanShard_ShardIsClosed_ShardClosedHandler(t *testing.T) {
	var client = &kinesisClientMock{
		getShardIteratorMock: func(input *kinesis.GetShardIteratorInput) (*kinesis.GetShardIteratorOutput, error) {
			return &kinesis.GetShardIteratorOutput{
				ShardIterator: aws.String("49578481031144599192696750682534686652010819674221576194"),
	...
	c, err := New("myStreamName", WithClient(client), WithStore(fakeStore{shardClosedErr: fmt.Errorf("closed shard error")}))
	if err != nil {
		t.Fatalf("new consumer error: %v", err)
	}
...
type fakeStore struct {
	shardClosedErr error
}

func (store fakeStore) GetCheckpoint(streamName, shardID string) (string, error) {
	return "", nil
}

func (store fakeStore) SetCheckpoint(streamName, shardID, sequenceNumber string) error {
	return nil
}

func (store fakeStore) ShardClosed(streamName, shardID string) error {
	return store.shardClosedErr
}
